"""Данный скрипт обеспечивает парсинг "тестового" сайта "www.spr.ru".
...
##### FIX 1.0 | Замена локальных ссылок на внешние | Const
REPLACING_PARAMETER_STRING_SAIT = "https://calorizator.ru"
REPLACING_PARAMETER_STRING_MASK = "/site"
REPLACING_PARAMETER_STRING = REPLACING_PARAMETER_STRING_SAIT + "/site"
#####


# FIX 1.0 | Замена локальных ссылок на внешние
def replacing_local_links(source_code):
    return source_code.replace(REPLACING_PARAMETER_STRING_MASK, REPLACING_PARAMETER_STRING)


response = get(URL, headers=HEADERS_DICT)

##### FIX 1.0 | Замена локальных ссылок на внешние
ress = replacing_local_links(response.text)
#####
...
def replace_char(name):
    """Функция, заменяющая символы из строки, на '_', если они находятся в кортеже недопустимых знаков.
...
def create_files(dictionary):
    ...
    for name, href in dictionary.items():
        if page == 0:
            ...
            ##### FIX 1.0 | Замена локальных ссылок на внешние
            source = replacing_local_links(source)
            #####
            ...
            with open(f"data/{page}_{correct_name}.csv", 'w', encoding='utf-8-sig') as file_category:
                writer = csv.writer(file_category)
                writer.writerow(
